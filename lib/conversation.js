...
async function persistMessageLog(psid, direction, payload) {
  const task = logMessage(psid, direction, payload);
  if (deferWebMessengerTask(psid, task)) return;
  await task;
}
...
async function replyAndLog(psid, text, quickReplies = []) {
  ...
  await persistMessageLog(psid, "out", {
    senderType: "bot",
    text: finalText,
...
async function maybeSendAfterHoursNotice(psid, existing, state, context, text) {
  ...
  // Website visitors already see an opening message in the widget. Avoid an
  // extra blocking write + alert before answering their first actual question.
...
async function answerFaq(psid, intent, context, rawText = "") {
  ...
  if (intent === "privacy") {
    await replyAndLog(psid, chooseCopy([
      "Homestay Phú Nhuận hướng đến trải nghiệm nghỉ ngơi riêng tư. Với câu hỏi cụ thể về an ninh hoặc khu vực camera, mình sẽ chuyển nhân viên xác nhận chính xác cho bạn.",
...
async function handleConversationEventInternal({
  ...
  await persistMessageLog(psid, "in", { text, payload, eventId, attachmentType });
  ...
  if (!payload && detectGoodbye(text)) {
    await replyAndLog(psid, chooseCopy([
      ...
      "Cảm ơn bạn đã liên hệ Homestay Phú Nhuận. Hẹn gặp lại bạn nhé."
    ]));
    return;
  }
