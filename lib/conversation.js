...
async function persistConversationLog(psid, direction, payload) {
  const task = logMessage(psid, direction, payload);
  if (isWebsiteConversation(psid)) {
    task.catch(error => console.warn("Website chat log skipped after response", {
      direction,
      error: String(error?.message || error || "log_failed")
    }));
    return;
  }
  await task;
...
async function rememberOutboundWhenNeeded(psid, messageId, type) {
  // Website messages are never echoed back by Meta, so storing their generated
  // message IDs for Messenger echo detection is unnecessary and slow.
  if (isWebsiteConversation(psid)) return;
  await rememberBotOutboundMessage(psid, messageId, type).catch(() => {});
}

function customerActorName(psid = "") {
  return isWebsiteConversation(psid) ? "Khách website" : "Khách Messenger";
}

function customerChannelLabel(psid = "") {
  return isWebsiteConversation(psid) ? "Website AI Chat" : "Messenger";
}

function customerReplyChannel(psid = "") {
  return isWebsiteConversation(psid) ? "khung chat này" : "Messenger";
}

function trackingParams(psid = "") {
  return isWebsiteConversation(psid)
    ? { src: "website", medium: "ai_chat", campaign: "chatbot" }
    : { src: "facebook", medium: "messenger", campaign: "chatbot" };
}

function buildSiteUrl(psid = "") {
  const params = new URLSearchParams(trackingParams(psid));
  return `${config.siteUrl}/?${params.toString()}`;
}

function buildGalleryUrl(psid = "") {
  const params = new URLSearchParams(trackingParams(psid));
  return `${config.siteUrl}/hinh-anh?${params.toString()}`;
}
...
function buildCalendarUrl(context = {}, psid = "") {
  const params = new URLSearchParams(trackingParams(psid));
...
function buildRoomUrl(home, context = {}, psid = "") {
  const params = new URLSearchParams(trackingParams(psid));
  if (context.dateKey) params.set("date", context.dateKey);
  if (Number.isFinite(context.startMinute)) params.set("start", minuteToLabel(context.startMinute));
  if (Number.isFinite(context.checkoutMinute)) params.set("checkout", minuteToLabel(context.checkoutMinute));
...
async function replyAndLog(psid, text, quickReplies = []) {
  ...
  const skipWebsiteRewrite = isWebsiteConversation(psid)
    && config.webChatFastMode
    && config.webChatGeminiRewrite !== true;

  // The deterministic reply already contains verified Firebase data. On the
  // website, waiting for a second Gemini call only changes wording and can add
  // several seconds. Messenger keeps the existing rewrite behavior.
  const finalText = skipWebsiteRewrite
    ? originalText
    : await rewriteReplyWithGemini({
  ...
  await rememberOutboundWhenNeeded(psid, result?.message_id, "text");
  await persistConversationLog(psid, "out", {
    senderType: "bot",
    text: finalText,
...
async function sendTemplateAndLog(psid, text, buttons, type = "template") {
  ...
  await rememberOutboundWhenNeeded(psid, result?.message_id, type);
  await persistConversationLog(psid, "out", {
    senderType: "bot",
    type,
    text,
...
async function sendImagesAndLog(psid, imageUrls, home = {}) {
  ...
  await rememberOutboundWhenNeeded(
    psid,
    result?.message_id,
    "image_gallery"
  );
...
async function askHomeForGallery(psid, context = {}) {
  ...
  if (!replies.length) {
    await sendTemplateAndLog(psid, "HOME chưa đọc được danh sách phòng. Bạn có thể mở thư viện ảnh tại đây.", [
      { type: "web_url", title: "Mở thư viện", url: buildGalleryUrl(psid) },
      { type: "web_url", title: "Xem lịch trống", url: buildCalendarUrl(context, psid) }
    ], "gallery");
    return;
  }
...
async function sendHomeGallery(psid, homeReference, context = {}) {
  ...
  if (!images.length) {
    await sendTemplateAndLog(
      ...
      [
        { type: "web_url", title: "Xem phòng", url: buildRoomUrl(home, context, psid) },
        { type: "web_url", title: "Xem lịch trống", url: buildCalendarUrl({ ...context, preferredHomeId: home.homeId }, psid) }
      ],
      "gallery_empty"
    );
    return;
  ...
  await sendTemplateAndLog(
    ...
    [
      { type: "web_url", title: "Xem đầy đủ", url: buildRoomUrl(home, nextContext, psid) },
      { type: "web_url", title: "Xem lịch trống", url: buildCalendarUrl(nextContext, psid) }
    ],
    "home_gallery"
  );
}
...
async function maybeSendAfterHoursNotice(psid, existing, state, context, text) {
  ...
  await safeNotifyOwner(psid, {
    ...
    message: text || `Khách bắt đầu cuộc trò chuyện ${customerChannelLabel(psid)} ngoài giờ.`,
    context: alertContext(context),
    afterHours: true
  });
}
...
async function showAvailability(psid, context) {
  const fastWebsite = isWebsiteConversation(psid) && config.webChatFastMode;
  const liveCheck = await checkLiveAvailability({
    ...context,
    // Reuse a very short cache on website chat. The cache is intentionally
    // small (default 15 seconds) so booking answers remain practically live.
    forceRefresh: !fastWebsite
  });
...
async function showSelectedHome(psid, context, homeId) {
  ...
  await sendTemplateAndLog(
    ...
    [
      { type: "web_url", title: "Xem phòng", url: buildRoomUrl(home, nextContext, psid) },
      { type: "web_url", title: "Xem lịch trống", url: buildCalendarUrl(nextContext, psid) },
...
async function handoff(psid, context) {
  ...
  await setConversationControl(psid, "human", {
    ...
    actorName: customerActorName(psid),
    source: "conversation",
...
async function answerPriceFaq(psid, context, rawText = "") {
  const catalog = await loadPricingCatalog({ forceRefresh: !(isWebsiteConversation(psid) && config.webChatFastMode) });
...
async function answerFaq(psid, intent, context, rawText = "") {
  ...
  if (intent === "location") {
    ...
    await sendTemplateAndLog(
      psid,
      `${label} ${address}. Khu vực Phú Nhuận, thuận tiện di chuyển đến các quận trung tâm.`,
      [
        { type: "web_url", title: "Mở Google Maps", url: mapUrl },
        { type: "web_url", title: home ? "Xem HOME này" : "Xem website", url: home ? buildRoomUrl(home, context, psid) : buildSiteUrl(psid) },
        { type: "web_url", title: "Xem lịch trống", url: buildCalendarUrl(context, psid) }
      ],
      "faq_location"
    );
  ...
  if (intent === "checkin") {
    await replyAndLog(
      psid,
      `Sau khi xác nhận thanh toán, bên mình sẽ gửi hướng dẫn nhận phòng và thông tin cần thiết qua ${customerReplyChannel(psid)}. Bạn nhớ chọn đúng ngày, giờ và HOME trước nha.`,
      [{ title: "Kiểm tra lịch", payload: "START|AVAILABILITY" }]
    );
    return true;
  }
  ...
  if (intent === "privacy") {
    await replyAndLog(psid, chooseCopy([
      "Homestay Quận Phú Nhuận hướng đến trải nghiệm nghỉ ngơi riêng tư. Với câu hỏi cụ thể về an ninh hoặc khu vực camera, mình sẽ chuyển nhân viên xác nhận chính xác cho bạn.",
  ...
  if (intent === "self_checkin") {
    await replyAndLog(psid, `Sau khi booking và thanh toán được xác nhận, bên mình sẽ gửi hướng dẫn nhận phòng, thông tin phòng và cách vào phòng qua ${customerReplyChannel(psid)}. Trường hợp cần hỗ trợ trực tiếp, nhân viên vẫn theo dõi hội thoại.`, [
      { title: "Kiểm tra lịch", payload: "START|AVAILABILITY" },
      { title: "Gặp nhân viên", payload: "HUMAN|REQUEST" }
    ]);
    return true;
...
async function handleConversationEventInternal({
  ...
  await persistConversationLog(psid, "in", { text, payload, eventId, attachmentType });
  ...
  if (state === "human_handoff") {
    ...
    if (phone) {
      ...
      await saveLead(psid, { status: "needs_human", phone, handoffRequested: true, afterHours: isAfterHours() }, { critical: true });
      await safeNotifyOwner(psid, {
        type: "human_phone",
        priority: "high",
  ...
  if (!availabilityIntent && detectCalendarLinkRequest(text) && !payload && !parsedDate && parsedTime == null && !parsedDuration) {
    await sendTemplateAndLog(psid, "Mình gửi bạn lịch trống hiện tại nhé.", [
      { type: "web_url", title: "Mở lịch trống", url: buildCalendarUrl(context, psid) }
    ], "calendar_link");
    return;
  }
  ...
  if (!payload && detectGoodbye(text)) {
    await replyAndLog(psid, chooseCopy([
      ...
      "Cảm ơn bạn đã liên hệ Homestay Quận Phú Nhuận. Hẹn gặp lại bạn nhé."
    ]));
    return;
  }
  ...
  if (state === "awaiting_phone") {
    ...
    if (!phone) {
      await replyAndLog(psid, "Số điện thoại chưa đúng định dạng. Bạn nhập lại giúp mình, ví dụ 0933882896 nhé.");
      return;
    }
    ...
    await replyAndLog(
      psid,
      `Mình đã ghi nhận số ${phone}. Nhân viên sẽ kiểm tra lần cuối và phản hồi ${isAfterHours() ? `từ ${afterHoursResumeLabel()}` : "ngay"} qua ${customerReplyChannel(psid)}. Lịch chỉ được khóa sau khi xác nhận thanh toán.${config.overnightIdRequired && isOvernightDuration(context.durationHours) ? " Gói qua đêm cần gửi CCCD để xác nhận thông tin nhận phòng." : ""}`
    );
    return;
  }
