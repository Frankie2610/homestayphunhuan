...
function homeEntries(homesObject) {
  return Object.entries(homesObject || {})
    ...
    .map(([homeId, home]) => [homeId, normalizePublicHomeRecord(home)]);
}
...
async function loadHomes({ force = false } = {}) {
  ...
  const snap = await getAdminDatabase().ref("homes").get();
...
async function loadMonth(monthPath, { force = false } = {}) {
  ...
  const snap = await getAdminDatabase().ref(`bookingsByMonth/${monthPath}`).get();
...
function publicHome(homeId, home, index) {
  ...
  const address = firstPresent(home, [
    "address",
    "addressLine",
    "location",
    "diaChi",
  ...
  const mapUrl = firstPresent(home, [
    "mapUrl",
    "googleMapsUrl",
    "googleMapUrl",
    "mapsUrl"
  ]) || config.mapUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`;
...
async function readAvailabilityData(start, end, { forceRefresh = true } = {}) {
  const homesObject = await loadHomes({ force: forceRefresh });
  ...
  const bookings = (await Promise.all(
    months.map(month => loadMonth(month, { force: forceRefresh }))
  )).flat();
...
export async function suggestAvailableSlots({
  ...
  const data = await readAvailabilityData(rangeStart, rangeEnd, { forceRefresh: true });
