...
function migratedEnv(name, fallback, legacyValues = []) {
  const value = String(process.env[name] || "").trim();
  if (!value || legacyValues.includes(value)) return fallback;
  return value;
}

export const config = {
  timezone: process.env.BOT_TIMEZONE || "Asia/Ho_Chi_Minh",
  brandName: migratedEnv("BOT_BRAND_NAME", "Homestay Phú Nhuận", ["Homestay 3 Cây Non", "3 Cây Non"]),
  siteUrl: migratedEnv(
    "PUBLIC_SITE_URL",
    "https://homestayphunhuan.vercel.app",
    ["https://homestay3caynon.vercel.app", "https://homestay3caynon.vercel.app/"]
  ).replace(/\/$/, ""),
  supportPhone: migratedEnv("BOT_SUPPORT_PHONE", "0933882896", ["0902932808", "+84902932808"]),
  homeAddress: migratedEnv(
    "BOT_HOME_ADDRESS",
    "26/10 Lê Văn Sỹ, Quận Phú Nhuận, TP.HCM",
    [
      "23/5/18 Lê Văn Duyệt, Phường Gia Định, TP.HCM",
      "23/5/18 Lê Văn Duyệt, Gia Định, TP.HCM"
    ]
  ),
  mapUrl: migratedEnv(
    "BOT_MAP_URL",
    "https://www.google.com/maps/search/?api=1&query=26%2F10%20L%C3%AA%20V%C4%83n%20S%E1%BB%B9%2C%20Qu%E1%BA%ADn%20Ph%C3%BA%20Nhu%E1%BA%ADn%2C%20TP.HCM",
    ["https://maps.app.goo.gl/9LdGei2sFCZ9dsRD9"]
  ),
  ...
  businessProfileCacheMs: envNumber("BOT_BUSINESS_PROFILE_CACHE_MS", 30_000),
  ...
  websiteGuestAlertEnabled: envBoolean("BOT_WEBSITE_GUEST_ALERT_ENABLED", true),
