...
function captureWebMessage(recipientId, message) {
  ...
  const capturedMessage = { id: messageId, ...message };
  capture.messages.push(capturedMessage);
  try {
    capture.onMessage?.(capturedMessage);
  } catch (error) {
    // A disconnected browser must not interrupt the shared conversation logic.
    console.warn("Website chat stream disconnected", {
...
/**
 * Runs non-critical website persistence alongside the reply. The capture waits
 * for these tasks before closing its stream, but sent messages are delivered
 * to the browser immediately through onMessage.
 */
export function deferWebMessengerTask(recipientId, task) {
  const capture = getWebCapture(recipientId);
  if (!capture) return false;

  const guardedTask = Promise.resolve(task).catch(error => {
    console.error("Website chatbot background task failed", {
      error: error?.message || String(error)
    });
  });
  const scheduled = capture.onBackgroundTask?.(guardedTask) === true;
  if (!scheduled) capture.backgroundTasks.push(guardedTask);
  return true;
}
 ...
 */
export function runWithWebMessengerCapture(
  recipientId,
  handler,
  { onMessage, onBackgroundTask } = {}
) {
  const capture = {
    recipientId,
    messages: [],
    onMessage: typeof onMessage === "function" ? onMessage : null,
    onBackgroundTask: typeof onBackgroundTask === "function" ? onBackgroundTask : null,
    backgroundTasks: []
  };
  return webMessengerStorage.run(capture, async () => {
    const result = await handler();
    await Promise.all(capture.backgroundTasks);
